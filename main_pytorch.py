...
class model():
    def __init__(self):
        args.dataset='IJCAI_15'
        self.train_file = args.path + args.dataset + '/trn_'
        self.test_file = args.path + args.dataset + '/tst_int'
        ...
        for i in range(0, len(self.behaviors)):
            with open(self.train_file + self.behaviors[i], 'rb') as fs:
                data = pickle.load(fs)
        ...
        self.A= graph_utils.get_composite(self.behaviors,self.behavior_mats, self.A)
        time = datetime.datetime.now()
        ...
        print("\n")
        # 首先，计算非零元素的索引，并确保self.trianMat是一个稀疏矩阵
        train_u, train_v = self.trainMat.nonzero()
        # 提取训练集索引
        train_data = np.hstack((train_u.reshape(-1, 1), train_v.reshape(-1, 1))).tolist()
        train_dataset = graph_utils.RecDataset_beh(self.behaviors, train_data, self.item_num,
                                             self.behaviors_data, True)
        self.train_loader = dataloader.DataLoader(train_dataset, batch_size=args.batch, shuffle=True, num_workers=4,
                                                  pin_memory=True)

        with open(self.test_file, 'rb') as fs:
            data = pickle.load(fs)
        ...
        test_dataset = graph_utils.RecDataset(test_data, self.item_num, self.trainMat, 0, False)
        self.test_loader = dataloader.DataLoader(test_dataset, batch_size=args.test_batch, shuffle=False, num_workers=4,
                                                 pin_memory=True)
    def prepareModel(self):
        ...
        if args.isload == True:
            ...
            self.meta_weight_net = MV_Net.MetaWeightNet(len(self.behaviors)).cuda(device=args.device)
        else:
            self.model = Model.myModel(self.user_num, self.item_num, self.behaviors, self.behavior_mats, self.A).cuda(device=args.device)
            self.meta_weight_net = MV_Net.MetaWeightNet(len(self.behaviors)).cuda(device=args.device)

        #IJCAI_15
        self.opt = t.optim.AdamW(self.model.parameters(), lr=args.lr, weight_decay=args.opt_weight_decay)
        self.meta_opt = t.optim.AdamW(self.meta_weight_net.parameters(), lr=args.meta_lr,
        ...
        self.meta_scheduler = t.optim.lr_scheduler.CyclicLR(self.meta_opt, args.meta_opt_base_lr, args.meta_opt_max_lr,
                                                            step_size_up=2, step_size_down=3, mode='triangular',
...
# ----------------------------------------------------------------------------------------------------------------------
        ...
        for user, item_i, item_j in tqdm(train_loader):
            user = user.long().cuda(device=args.device)
            self.user_step_index = user
            self.meta_user = t.as_tensor(self.meta_multi_single[self.meta_start_index:self.meta_end_index]).cuda(device=args.device)
            ...
            meta_model = Model.myModel(self.user_num, self.item_num, self.behaviors, self.behavior_mats,self.A).cuda(device=args.device)
...
#对于所有的行为，把行为中用户存在交互的索引取出来，然后赋给embedding
            for index in range(len(self.behaviors)):
                not_zero_index = np.where(item_i[index].cpu().numpy() != -1)[0]

                self.user_id_list[index] = user[not_zero_index].long().cuda(device=args.device)
                meta_user_index_list[index] = self.user_id_list[index]
                self.item_id_pos_list[index] = item_i[index][not_zero_index].long().cuda(device=args.device)
                self.item_id_neg_list[index] = item_j[index][not_zero_index].long().cuda(device=args.device)
                ...
                #   BPRloss(meta_pred_i.view(-1) - meta_pred_j.view(-1))：计算meta_pred_i和meta_pred_j中对应元素的差值。
                # 这个差值应该越大越好，表示模型更偏好meta_pred_i对应的用户-物品对。
                meta_behavior_loss_list[index] = - (meta_pred_i.view(-1) - meta_pred_j.view(-1)).sigmoid().log()

            meta_infoNCELoss_list, SSL_user_step_index = self.SSL(meta_user_embeds, meta_item_embeds, meta_user_embed,
                                                                  meta_item_embed, self.user_step_index)
            ...
            nn.utils.clip_grad_norm_(self.meta_weight_net.parameters(), max_norm=10, norm_type=2)
            nn.utils.clip_grad_norm_(meta_model.parameters(), max_norm=10, norm_type=2)
            meta_opt.step()
            self.meta_opt.step()
            ...
            for index in range(len(self.behaviors)):
                user_id, item_id_pos, item_id_neg = self.sampleTrainBatch(t.as_tensor(self.meta_user),
                                                                          self.behaviors_data[i])
            ...
            self.infoNCELoss_list, SSL_user_step_index = self.SSL(user_embeds, item_embeds, user_embed, item_embed,
                                                                  self.meta_user)
            ...
            infoNCELoss_list, SSL_user_step_index = self.SSL(user_embeds, item_embeds, user_embed, item_embed,
                                                             self.user_step_index)
    ...
    def sampleTestBatch(self, batch_user_id, batch_item_id):
        ...
        for i in range(batch):
            ...
            # 看一下这些负样本商品与其他用户的交集数量
            pvec = self.labelP[negset]
            pvec = pvec / np.sum(pvec)
#选择99个负样本，这些负样本与user i 不存在buy的交集，但是却又是所有用户buy最多的物品
...
#shoot_index只是索引，具体对应的哪个物品要去user_item100中找
    ...
    def sampleTrainBatch(self, batIds, labelMat):
        ...
        return t.as_tensor(np.array(user_id)).cuda(device=args.device), t.as_tensor(np.array(item_id_pos)).cuda(device=args.device), t.as_tensor(
            np.array(item_id_neg)).cuda(device=args.device)
    ...
    def SSL(self, user_embeddings, item_embeddings, target_user_embeddings, target_item_embeddings, user_step_index):
        ...
        def neg_sample_pair(x1, x2, τ = 0.05):
            for i in range(x1.shape[0]):
                ...
                index_set.remove(i)
                index_set_neg = t.as_tensor(np.array(list(index_set))).long().cuda(device=args.device)
        ...
        def one_neg_sample_pair_index(i, step_index, embedding1, embedding2):
            ...
            neg2_index = t.as_tensor(np.array(list(index_set))).long().cuda(device=args.device)
        ...
        def multi_neg_sample_pair_index(batch_index, step_index, embedding1, embedding2):  #small, big, target, beh: [100], [1024], [31882, 16], [31882, 16]
            ...
            neg2_index = t.as_tensor(np.array(list(neg2_index_set))).long().cuda(device=args.device)  #[910]
            ...
            neg2_index = t.squeeze(neg2_index)                                   #[91000]
                                                                                 #target
            neg1_index = batch_index.long().cuda(device=args.device)     #[100]
        ...
        def single_infoNCE_loss_simple(embedding1, embedding2):
            ...
            one = t.cuda(device=args.device).FloatTensor(neg1.shape[0]).fill_(1)  #[100]
        ...
        def single_infoNCE_loss_one_by_one(embedding1, embedding2, step_index):  #target, beh
            ...
            neg_score = t.zeros((N,), dtype = t.float64).cuda(device=args.device)  #[1024]
        ...
        user_step_index = t.as_tensor(np.random.choice(user_step_index.cpu(), size=SSL_len, replace=False, p=None)).cuda(device=args.device)
    ...
    def saveHistory(self):
        ...
        with open(r'/data/ssx/IJCAI/ablation/' + ModelName + '.his', 'wb') as fs:
    ...
    def saveModel(self):
        ...
        history['HR'] = self.his_hr
        history['NDCG'] = self.his_ndcg
        savePath = r'/data/ssx/IJCAI/ablation/' + ModelName + r'.pth'
        params = {
            'epoch': self.curEpoch,
